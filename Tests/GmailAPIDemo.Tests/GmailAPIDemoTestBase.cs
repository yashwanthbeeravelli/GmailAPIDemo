...
namespace GmailAPIDemo.Tests
{
    public abstract class GmailAPIDemoTestBase : AbpIntegratedTestBase<GmailAPIDemoTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected GmailAPIDemoTestBase()
        {
        ...
        protected void UsingDbContext(Action<GmailAPIDemoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<GmailAPIDemoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<GmailAPIDemoDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<GmailAPIDemoDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<GmailAPIDemoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GmailAPIDemoDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<GmailAPIDemoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GmailAPIDemoDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<GmailAPIDemoDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<GmailAPIDemoDbContext, Task<T>> func)
        {
            T result;
